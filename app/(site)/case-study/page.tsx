    ...
    <>
      ...
      <div className="flex flex-col gap-8">
        ...
        <Decision
          ...
          tradeoff={
            <>
              It means real domain logic lives in SQL, not TypeScript, which is
              harder to unit test with the usual tooling, and it only shows up if
              a reviewer opens the migration file rather than clicking around the
              UI. Worth it anyway: it&apos;s the single strongest proof that this
              isn&apos;t just CRUD with an admin panel bolted on.
            </>
          }
        />
        ...
        <Decision
          number="03"
          title="Accessibility is audited, not asserted"
          chosen={
            <>
              Before deploying, every public page went through Lighthouse
              (mobile and desktop) plus a full{" "}
              <code className="font-mono text-sm text-ink">axe-core</code>{" "}
              pass via Playwright, a broader rule set than the subset
              Lighthouse&apos;s own accessibility score samples, and the reason it
              caught what Lighthouse didn&apos;t. Every public page now scores 100
              on accessibility, best-practices and SEO, with zero axe-core
              violations.
            </>
          }
          why={
            <>
              The audit found three real bugs, not zero. A skip link
              pointed at an id only the homepage actually had, so it was a
              dead link on six of seven pages. Two elements had an{" "}
              <code className="font-mono text-sm text-ink">aria-label</code>{" "}
              that didn&apos;t match their visible text, so a screen reader announced
              something a sighted user wouldn&apos;t hear said. And every menu card
              nested interactive elements: a clickable card wrapping a real{" "}
              <code className="font-mono text-sm text-ink">&lt;button&gt;</code>{" "}
              inside it, which is invalid markup regardless of whether the
              click handlers still technically worked.
            </>
          }
          tradeoff={
            <>
              The menu card fix needed a real restructure, not a label
              change. The two controls are now siblings: an invisible
              full-card button underneath for &quot;view details,&quot; the
              add-to-cart button layered on top via{" "}
              <code className="font-mono text-sm text-ink">
                pointer-events
              </code>{" "}
              rather than z-index alone, since a later sibling normally
              hit-tests over an earlier one regardless. Verified with a live
              Playwright check, not just the audit, that both click targets
              still do the right thing after the change.
            </>
          }
        />
