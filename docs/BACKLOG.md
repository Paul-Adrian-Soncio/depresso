...
| Performance & a11y report | `S` | ✅ **Audit done**, real figures recorded in `docs/DECISIONS.md` — every public page scores 100 on accessibility/best-practices/SEO and 0 axe-core violations, after fixing a broken skip link on 6 of 7 pages, two `aria-label` mismatches, and a nested-interactive-elements bug on the menu cards found by the audit itself. Performance is high-80s–high-90s on Lighthouse's throttled mobile preset, 98–100 desktop. Numbers still need to land in the README, not just here. |
